...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['file'])) {
        ...
        if ($file_error === 0) {
            ...
            $allowed = array('png','jpg','webp','pdf', 'doc', 'docx', 'txt','sql','ppt','pptx','zip','rar','mp3','mp4');

            // Check if file type is allowed
            if (in_array($file_ext, $allowed)) {
                $new_file_name = $file_name;
                $destination = "admin/uploads/documents/" . $new_file_name; // Adjust destination path as per your file structure

                // Move uploaded file to destination
                if (move_uploaded_file($file_tmp, $destination)) {
                    // Prepare and execute the SQL statement to insert data into the database
                    $sql = "INSERT INTO documents (title, description, file, project_id) VALUES (?, ?, ?, ?)";
                    $stmt = mysqli_prepare($conn, $sql);
                    mysqli_stmt_bind_param($stmt, "sssi", $title, $description, $new_file_name, $project);
                    $result = mysqli_stmt_execute($stmt);

                    if ($result) {
                        header("Location: document.php?msg=Document added successfully");
                        exit();
                    } else {
                ...
                <form method="post" enctype="multipart/form-data" action="">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control mb-3" name="project">
                            ...
                            $no = 1;
                            $sql2 = "SELECT p.*, d.name AS department_name 
                            FROM project p 
                            ...
                            WHERE u.id = $user_id;
                            ";
                            $project = mysqli_query($conn, $sql2);
